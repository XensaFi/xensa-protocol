...
interface IXensaMinterQuery {
    function getGroupInfo(uint256 _pid, uint256 _gid) external view returns (uint256 gp, uint256 groupTotal, uint256 accPerShare);
    function pendingXensa(uint256 _pid, uint256 _gid, address _user) external returns (uint256 amount, uint256 protectAmount, uint256 rewardDebt, uint256 pending, uint256 protectMintPending, uint256 protectPrice, uint256 lockedTotal); 
}

contract XensaQuery {
    using SafeMath for uint256;
    IXensaMinterQuery private mt;
    constructor(address addressXensaMint) public{
        mt = IXensaMinterQuery(addressXensaMint); 
    }
