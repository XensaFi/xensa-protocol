...
/**
* @title XensaLiquidationManager contract
* @notice Implements the liquidation function.
**/
contract XensaLiquidationManager is ReentrancyGuard, Ownable, VersionedInitializable {
    ...
    XensaCore core;
    XensaDataProvider dataProvider;
    XensaParametersProvider parametersProvider;
    ...
    **/
    ...
    function init(XensaAddressesProvider _addressesProvider) external onlyOwner {
    ...
    * of the Xensa contract, the getRevision() function is needed.
    */
    ...
    **/
    function liquidationCall(
        ...
        XToken collateralXToken = XToken(core.getReserveXTokenAddress(_collateral));

        //if liquidator reclaims the xToken, he receives the equivalent xtoken amount
        if (_receiveXToken) {
            collateralXToken.transferOnLiquidation(_user, _caller, maxCollateralToLiquidate);
        } else {
            //otherwise receives the underlying asset
            //burn the equivalent amount of xtoken
            collateralXToken.burnOnLiquidation(_user, maxCollateralToLiquidate);
            core.transferToUser(_collateral, _caller, maxCollateralToLiquidate);
        }

        //transfers the principal currency to the pool
        core.transferToReserve.value(msg.value)(_reserve, _caller, vars.actualAmountToLiquidate);

        if (vars.feeLiquidated > 0) {
            //if there is enough collateral to liquidate the fee, first transfer burn an equivalent amount of
            //xTokens of the user
            collateralXToken.burnOnLiquidation(_user, vars.liquidatedCollateralForFee);
    ...
    **/
    function calculateAvailableCollateralToLiquidate(
        ...
        vars.collateralUnit = core.getReserveUnit(_collateral);
        vars.principalUnit = core.getReserveUnit(_principal);
        ...
        vars.maxAmountCollateralToLiquidate = vars.maxAmountCollateralToLiquidate
            .div(vars.principalUnit)
            .mul(vars.collateralUnit);
        ...
        if (vars.maxAmountCollateralToLiquidate > _userCollateralBalance) {
            ...
            principalAmountNeeded = principalAmountNeeded 
                .div(vars.collateralUnit)
                .mul(vars.principalUnit);
        } else {
