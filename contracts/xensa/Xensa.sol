...
import "../configuration/XensaParametersProvider.sol";
...
import "./XensaDataProvider.sol";
import "./XensaLiquidationManager.sol";
import "../libraries/EthAddressLib.sol";

/**
* @title Xensa contract
* @notice Implements the actions of the Xensa, and exposes accessory methods to fetch the users and reserve data
* @author Xensa
 **/

contract Xensa is VersionedInitializable {
    using SafeMath for uint256;
    using WadRayMath for uint256;
    using Address for address;

    XensaAddressesProvider public addressesProvider;
    XensaCore public core;
    XensaDataProvider public dataProvider;
    XensaParametersProvider public parametersProvider;
    IFeeProvider feeProvider;
    XensaLiquidationManager public liquidationManager;
    IXensaMinter minter;
    ...
    **/
    ...
    /**
    * @dev these events are not emitted directly by the Xensa
    * but they are declared here as the XensaLiquidationManager
    * @param _reserve the address of the reserve
    ...
    * This allows to have the events in the generated ABI for Xensa.
    **/
    ...
    * @param _receiveXToken true if the liquidator wants to receive XTokens, false otherwise
    * @param _timestamp the timestamp of the action
    **/
    ...
    * @param _reserve the address of the reserve
    **/
    modifier onlyOverlyingXToken(address _reserve) {
        require(
            msg.sender == core.getReserveXTokenAddress(_reserve),
            "The caller of this function can only be the XToken contract of this reserve"
        );
        _;
    }
    ...
    **/
    ...
    /**
    * @dev this function is invoked by the proxy contract when the Xensa contract is added to the
    * AddressesProvider.
    * @param _addressesProvider the address of the XensaAddressesProvider registry
    **/
    function initialize(XensaAddressesProvider _addressesProvider) public initializer {
         _notEntered = true;
        addressesProvider = _addressesProvider;
        core = XensaCore(addressesProvider.getXensaCore());
        dataProvider = XensaDataProvider(addressesProvider.getXensaDataProvider());
        parametersProvider = XensaParametersProvider(
            addressesProvider.getXensaParametersProvider()
        );
        feeProvider = IFeeProvider(addressesProvider.getFeeProvider());
        minter = IXensaMinter(addressesProvider.getXensaMinter());
    }

    /**
    * @dev deposits The underlying asset into the reserve. A corresponding amount of the overlying asset (XTokens)
    * is minted.
    * @param _reserve the address of the reserve
    * @param _amount the amount to be deposited
    ...
    **/
    ...
    {
        XToken xToken = XToken(core.getReserveXTokenAddress(_reserve));

        bool isFirstDeposit = xToken.balanceOf(msg.sender) == 0;

        core.updateStateOnDeposit(_reserve, msg.sender, _amount, isFirstDeposit);

        //minting XToken to user 1:1 with the specific exchange rate
        xToken.mintOnDeposit(msg.sender, _amount);
    ...
    * This function is executed by the overlying XToken contract in response to a redeem action.
    * @param _reserve the address of the reserve
    ...
    **/
    function redeemUnderlying(
        ...
        uint256 _amount,
        uint256 _xTokenBalanceAfterRedeem
    )
        external
        nonReentrant
        onlyOverlyingXToken(_reserve)
        onlyActiveReserve(_reserve)
        onlyAmountGreaterThanZero(_amount)
    {
        ...
        core.updateStateOnRedeem(_reserve, _user, _amount, _xTokenBalanceAfterRedeem == 0);
    ...
    * this is regulated by Xensa to ensure that the protocol is not abused, and the user is paying a fair
    ...
    * @param _receiveXToken true if the liquidators wants to receive the XTokens, false if
    ...
    **/
    function liquidationCall(
        ...
        liquidationManager = XensaLiquidationManager(addressesProvider.getXensaLiquidationManager());

        (uint256 success, string memory result) = liquidationManager.
            liquidationCall(
                _collateral,
                _reserve,
                _user,
